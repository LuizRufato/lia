...
const baseInput = () => ({
  ...
  offer: {
    status: 'ACTIVE',
    monetization: { status: 'VERIFIED', destinationUrl: 'https://shopee.test/offer' },
  },
  score: 0.8,
  observedAt: new Date(),
});
...
describe('WhatsAppSafetyGovernor', () => {
  ...
  it('blocks an offline session and disabled group', async () => {
    ...
    })).resolves.toMatchObject({ allowed: false, reason: 'WHATSAPP_SESSION_OFFLINE' });
    await expect(governor.evaluate({
      ...baseInput(),
      channel: { ...baseInput().channel, enabled: false },
    })).resolves.toMatchObject({ allowed: false, reason: 'WHATSAPP_GROUP_DISABLED' });
  });
  ...
  it('blocks kill switch, quiet window and expired observations', async () => {
    ...
    await expect(governor.evaluate({ ...baseInput(), now })).resolves.toMatchObject({
      allowed: false,
      reason: 'WHATSAPP_KILL_SWITCH',
    });
    const configWithoutKill = { ...config, killSwitch: false };
    const second = new WhatsAppSafetyGovernor(makePrisma(configWithoutKill) as any);
    await expect(second.evaluate({
      ...baseInput(),
      integration: { ...baseInput().integration, connectedAt: new Date(now.getTime() - 10 * 60_000) },
      now,
      observedAt: new Date(now.getTime() - 11 * 60_000),
    })).resolves.toMatchObject({ allowed: false, reason: 'WHATSAPP_OBSERVATION_EXPIRED' });
  });
  ...
  it('blocks daily limit and permits a healthy candidate', async () => {
    ...
    const blocked = new WhatsAppSafetyGovernor(makePrisma(config, [2, 2, null]) as any);
    await expect(blocked.evaluate(baseInput())).resolves.toMatchObject({
      allowed: false,
      reason: 'WHATSAPP_DAILY_LIMIT',
    });
    const allowed = new WhatsAppSafetyGovernor(makePrisma(config, [0, 0, null]) as any);
    await expect(allowed.evaluate(baseInput())).resolves.toEqual({ allowed: true });
  });
